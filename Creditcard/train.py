...
def create_loaders(X, y, batch_size, test_size=0.2, val_size=0.1, random_seed=42):
    ...
    def make_loader(X_arr, y_arr, shuffle=True):
        """
        Create a dataloader out of dataset
...
# Count total number of runs just for visualization
...
for ratio in config["imbalance_ratios"]:
    df_maj = df[df["Class"] == 0] # Majority class
    df_min = df[df["Class"] == 1] # Minority class
    keep_n = int(len(df_maj) * ratio) # Number of samples we want to keep to maintain the wanted imbalance ratio
    df_maj_sampled = df_maj.sample(keep_n, random_state=config["random_seed"]) # Samples we want to keep based on the imbalance ratio
    df_sub = pd.concat([df_min, df_maj_sampled], axis=0).sample(frac=1, random_state=config["random_seed"]) # Subset concatenated with the new samples

    # Split into Class and Samples
    X = df_sub.drop(columns=["Class"]).values.astype(np.float32)
    y = df_sub["Class"].values.astype(np.float32)

    # Create loaders from our new sampled dataset
    ...
    # # If we have multiple models iterate through them and all our loss functions
    for num_layer, loss_fn in itertools.product(config["num_layers"], config["losses"]):
        # Get our parameter configurations per loss
        if loss_fn == "focal":
        ...
        # Go through all the parameter combinations
        for params in param_combinations:
            # Prints for starting the run with metrics
            current_run += 1
            print("="*70)
            ...
            # Start new run with mlflow tracking
            with mlflow.start_run(run_name=run_name):
                ...
                # Initialize model and loss based on our current selected loss and hyperparameters
                ...
                for epoch in range(config["epochs"]):
                    ...
                    # Track loss for training and validation, as well as f1 score
                    ...
                    # Track the best F1 score so far
                ...
                # Prints for finishing the current run with evals
                print("-"*90)
                print(f"RUN {current_run}/{total_runs} FINISHED")
